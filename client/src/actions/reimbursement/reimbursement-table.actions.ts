...
export const fetchReimbursements = () => (dispatch: any) => {
  ...
  if (currentUser) {
    ...
    if (currentUser.role === "Manager") {
      ...
      fetchUrl = `${environment.context}users/${
        currentUser.userId
      }/reimbursements`;
    }
    fetch(fetchUrl, {
...
export const updateReimbursement = (
  ...
  if (currentUser && currentUser.role === "Manager") {
    fetch(`${environment.context}reimbursements/${reimbursementId}`, {
      body: JSON.stringify({
        resolved,
        resolver_id: currentUser.userId,
        status: newStatus
      }),
      credentials: "include",
      headers: {
...
export const filterReimbursements = (
...
export const updateActivePage = (
  ...
  const renderedReimbursements = filteredReimbursements.slice(
    startIndex,
    endIndex
  );
  dispatch({
